...
int main(int argc, char **argv)
{
  ...
  moveit_msgs::CollisionObject collision_object;
    for(int i=0; i < obstacle_names.size(); i++){
      if (less_collision_mask[i]){
        collision_object.id = obstacle_names[i];
        collision_object.header.frame_id = robot_ur5e.move_group_interface_arm->getPlanningFrame();
  ...
  robot_ur5e.planning_scene_interface.applyCollisionObjects(collision_objects);
  ...
  // EXTRA 1. Approach the chocolate bar
  ...
  approach_offset += chocolate_bar_size[1]*3/4;

  // EXTRA 2. Exiting the shelf
  std::cout << "EXITING THE SHELF..." << std::endl;
  tmp_pose = chosen_chocolate_bar_goal_pose;
  tmp_pose.position.x -= approach_offset;
  robot_ur5e.go_to_pose(chosen_chocolate_bar_goal_pose);

  // EXTRA 3. Approach the vision box
  std::cout << "APPROACHING THE VISON BOX..." << std::endl;
  ...
  // EXTRA 4. Exiting the vision box
  std::cout << "EXITING THE VISION BOX..." << std::endl;
  robot_ur5e.go_to_pose(tmp_pose);

  // EXTRA 5. Approach the portioning machine
  ...
  // EXTRA 4. Exiting the portioning machine
  std::cout << "EXITING THE PORTIONING MACHINE..." << std::endl;
  tmp_pose = portioning_machine_hole_goal_pose;
  tmp_pose.position.x += approach_offset;
  robot_ur5e.go_to_pose(tmp_pose);
